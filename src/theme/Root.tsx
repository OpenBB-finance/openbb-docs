...
import TerminalRedirect from "../components/General/TerminalRedirect";
		...
		posthog.init("phc_EqU3YjnV8OYmBlKanwWq222B8OHQksfmQBUtcVeteHR", {
			...
			loaded: () => {
				...
				posthog.onFeatureFlags(() => {
					...
					if (
						!posthog.isFeatureEnabled("collect-logs-web", { send_event: false })
					) {
	...
	useEffect(() => {
		if (
			...
			const cookie = document.cookie
				?.split(";")
				.find((c) => c.trim().startsWith("docs-login="));
