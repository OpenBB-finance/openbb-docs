...
export default function DocSidebarItem({ item, ...props }) {
	...
	const isRelevantSection = isPro || isExcel || isGettingStarted || isAgents;

	// Only show workspace items when in workspace or agents, excel items when in excel, getting-started items always in relevant sections
	if (item.type === "autogenerated") {
		// Hide ALL workspace directories and subdirectories when not in workspace
		if (item.dirName?.startsWith("workspace") && !(isPro || isAgents)) {
			return null;
		}
	...
	if (isSnowflake) {
		if (item.className?.includes("getting-started-section") ||
			item.className?.includes("agents-section") ||
	...
	// Show section headers only when in workspace, excel, getting-started, or agents sections
	...
	if (item.className?.includes("agents-section") && (!isRelevantSection || isPlatform || isCLI || isODP || isSnowflake)) {
		return null;
	}
	...
	// Only filter categories when in workspace/excel/getting-started/agents sections
