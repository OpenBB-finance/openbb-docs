...
Both are stored in the `extra` attribute of the [OBBject](/odp/python/developer/architecture_overview#obbject-class) response object.
...
This content can be disabled as a setting in the [`user_settings.json`](/odp/python/settings/environment_variables) file.
...
Enable LLM mode for the response object by configuring [`user_settings.json`](/odp/python/settings/user_settings), or for just the current Python session.
...
You can try other output [formatters](#results-formatters) such as `dict`, or you can build your own output handlers as [OBBject extensions](/odp/python/developer/extension_types/obbject)
...
Sections and length are defined in [`system_settings.json`](/odp/python/settings/system_settings).
