...
## Disabling Logging Service

The OpenBB Platform logs anonymized records of function calls, and any error messages produced, to a unique file for the session. They are written to `~/OpenBBUserData/logs`. 

The logging service can be disabled by setting "key":"value" pairs in `system_settings.json`. This file is located in the same folder as `user_settings.json`. Create the file if it does not exist.
